...
function mockServerModule(rpc?: (...args: unknown[]) => Promise<unknown>) {
  mock.module("@/lib/supabase/server", () => ({
    createServerClient: () => ({
      auth: {
        getUser: async () => ({
          data: { user: { id: "user-1", email: "pending@example.com", email_confirmed_at: null } },
          error: null,
        }),
      },
      rpc,
    ...
    createRefreshClient: () => ({
      auth: {
        refreshSession: async () => ({ data: { session: null }, error: new Error("not used") }),
      },
    }),
    supabaseAdmin: {
      from: () => ({
...
describe("auth hardening runtime", () => {
  ...
  it("awaits cookie clearing before redirecting revoked or suspended members", async () => {
    ...
    mock.module("@/api/auth", () => ({
      ...
      clearAuthCookieFn: () =>
        new Promise<{ ok: true }>((clear) => {
          releaseCookieClear = () => clear({ ok: true });
          notifyClearStarted?.();
        }),
    }));
    ...
    for (const status of ["removed", "suspended"] as const) {
      ...
      const clearStarted = new Promise<void>((resolve) => {
        notifyClearStarted = resolve;
      });
      const guard = checkAppGuardFn().then((result) => {
        guardResolved = true;
        return result;
      });
      await clearStarted;
